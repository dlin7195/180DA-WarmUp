...
while timer<200:
    ...
    if 1:                       #Change to '0' to stop  showing the angles from the Kalman filter
        outputString +="# timing %5.2f #" % (timer)
  #  print(outputString)
...
data = np.array([accXnorm, accYnorm])
f= open('/home/pi/Desktop/BerryIMU/python-BerryIMU-gyro-accel-compass-filters/Idle_data/data.csv','w')
writer=csv.writer(f)
writer.writerow(accxarr)
writer.writerow(accyarr)
writer.writerow(acczarr)
writer.writerow(gyrxarr)
writer.writerow(gyryarr)
writer.writerow(gyrzarr)
